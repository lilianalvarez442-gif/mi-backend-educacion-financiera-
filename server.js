...
function getGmailTransporter() {
  if (!GMAIL_USER || !GMAIL_APP_PASSWORD) {
    throw new Error("GMAIL_USER o GMAIL_APP_PASSWORD no configurados en el servidor.");
  }
...
async function sendViaGmail(toEmail, code) {
  const transporter = getGmailTransporter();
  await transporter.sendMail({
    from: `Educación Financiera <${GMAIL_USER}>`,
    to: toEmail,
    subject: `Tu código de verificación: ${code}`,
    html: `<p>Tu código de verificación para Educación Financiera es:</p><h2>${code}</h2><p>Vence en 10 minutos.</p>`,
  });
